...
final class SmartThresholdAdvisor {
    record Sample(Instant time, String source, double tps, double ramPercent) { }
    ...
    synchronized void record(Instant now, String source, double tps, double ramPercent) {
        if (!Double.isFinite(tps) && !Double.isFinite(ramPercent)) return;
        ...
        samples.add(new Sample(now, source, tps, ramPercent)); lastRecorded.put(source, now); prune(now); save();
    }
    ...
    private void load() {
        ...
        try { for (String line : Files.readAllLines(file, StandardCharsets.UTF_8)) { String[] p = line.split("\\|", 4); if (p.length == 4) samples.add(new Sample(Instant.ofEpochMilli(Long.parseLong(p[0])), new String(Base64.getUrlDecoder().decode(p[1]), StandardCharsets.UTF_8), Double.parseDouble(p[2]), Double.parseDouble(p[3]))); } prune(Instant.now()); for (Sample sample : samples) lastRecorded.merge(sample.source(), sample.time(), (a, b) -> a.isAfter(b) ? a : b); } catch (Exception ignored) { samples.clear(); lastRecorded.clear(); }
    }
    private void save() {
        if (file == null) return;
        try { Files.createDirectories(file.getParent()); List<String> lines = samples.stream().map(sample -> sample.time().toEpochMilli() + "|" + Base64.getUrlEncoder().withoutPadding().encodeToString(sample.source().getBytes(StandardCharsets.UTF_8)) + "|" + sample.tps() + "|" + sample.ramPercent()).toList(); Path temp = file.resolveSibling(file.getFileName() + ".tmp"); Files.write(temp, lines, StandardCharsets.UTF_8); try { Files.move(temp, file, StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.ATOMIC_MOVE); } catch (AtomicMoveNotSupportedException ignored) { Files.move(temp, file, StandardCopyOption.REPLACE_EXISTING); } } catch (IOException ignored) { }
    }
